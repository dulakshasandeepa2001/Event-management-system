...
const EditEvent = () => {
  ...
  const fetchEvent = async () => {
    try {
      ...
      setBatchId(event.batch?._id || event.batch || "");
      setTargetGroups(Array.isArray(event.targetGroups) && event.targetGroups.length ? event.targetGroups : ["All"]);
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              ...
              disabled={currentUser?.u_role === "batchrep" && !!myBatchId}
            >
          ...
          <div className="md:col-span-2 flex gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-3 rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-50"
            >
              <FaSave /> {saving ? "Saving..." : "Update Event"}
            </button>
